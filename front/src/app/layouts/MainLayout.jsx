          ...
          >
            ...
            <SidebarHeader className="px-4 group-data-[collapsible=icon]:px-2 py-5 flex flex-row items-center group-data-[collapsible=icon]:justify-center gap-3 border-b border-sidebar-border h-16 shrink-0">
              ...
              <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
                <h1 className="font-bold text-white leading-none tracking-wider text-sm truncate">
                  APP BIT
                </h1>
            ...
            <SidebarContent className="px-3 py-4 flex-1 overflow-y-auto">
              <SidebarMenu className="space-y-1">
                {menuItems.map((item) => {
                  ...
                  const isActive =
                    currentTab === item.id ||
                    (item.id === "inicio" && currentTab === "cluster-detail");
                  return (
                    <SidebarMenuItem key={item.id}>
                      ...
                      >
                        <Icon
                          className={`w-4 h-4 flex-shrink-0 ${isActive ? "text-[#3b82f6]" : "text-slate-400"}`}
                        />
                ...
                */}
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-1.5 text-slate-600 text-xs font-medium hover:text-slate-900 transition"
                  >
                    <Globe className="w-4 h-4" />
                    <span>{language === "es" ? "ES" : "PT"}</span>
                    <ChevronDown className="w-3.5 h-3.5 text-slate-400" />
                  </button>

                  {open && (
                    <div className="absolute right-0 mt-2 w-32 rounded-lg border border-slate-200 bg-white shadow-lg overflow-hidden z-50">
                      <button
                        onClick={() => {
                          setLanguage("es");
                          setOpen(false);
