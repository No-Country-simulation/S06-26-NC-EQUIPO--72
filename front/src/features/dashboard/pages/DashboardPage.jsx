...
function DashboardPage({ onTabChange, onClusterSelect }) {
  ...
  const barChartData = useMemo(() => {
    ...
    return rawEmpleo.regiones.map((r) => {
      ...
      return { region: cleanName, empleo, conectividad };
    });
  }, [rawEmpleo]);
  ...
  const currentAverages = useMemo(() => {
    ...
    if (regionesEmpleo.length) {
      const sumEmpleo = regionesEmpleo.reduce(
        (sum, r) =>
          sum + (r.indicadores?.[0]?.valor ? parseFloat(r.indicadores[0].valor) : 0),
        0,
      );
      ...
      const sumCongestion = regionesEmpleo.reduce(
        (sum, r) =>
          sum + (r.congestionamento_medio ? parseFloat(r.congestionamento_medio) : 0),
        0,
      );
  ...
  const lineChartConfig = {
    empleo: { label: isPortugues ? "Emprego" : "Empleo", color: "#2563eb" },
    conectividad: { label: isPortugues ? "Conectividade" : "Conectividad", color: "#0d9488" },
    inclusion: { label: isPortugues ? "Inclusão" : "Inclusión", color: "#a855f7" },
  };
  ...
  const pieChartData = useMemo(() => {
    ...
    regionesEmpleo.forEach((r) => {
      const congestion = r.congestionamento_medio
        ...
        : 0;
      if (congestion < 35) countOptimo++;
      else if (congestion <= 65) countAlerta++;
      else countCritico++;
    });
    ...
    return [
      { name: isPortugues ? "Com acesso digital" : "Con acceso digital", value: pctOptimo, color: "#2563eb" },
      { name: isPortugues ? "Brecha urbana-rural" : "Brecha urbana-rural", value: pctAlerta, color: "#f97316" },
      { name: isPortugues ? "Sem conectividade" : "Sin conectividad", value: pctCritico, color: "#ef4444" },
    ];
  }, [rawEmpleo]);

  const pieChartConfig = {
    acceso: { label: isPortugues ? "Com acesso digital" : "Con acceso digital", color: "#2563eb" },
    brecha: { label: isPortugues ? "Brecha urbana-rural" : "Brecha urbana-rural", color: "#f97316" },
    sinConectividad: { label: isPortugues ? "Sem conectividade" : "Sin conectividad", color: "#ef4444" },
  };
  ...
  const saludMentalData = useMemo(() => {
    ...
    const list = regionesSalud.map((r) => {
      ...
      return { original: r, cluster: r.cluster, rate };
    });
    ...
    return top4.map((item) => {
      ...
      if (score >= 4.0) color = "bg-red-500";
      else if (score >= 3.0) color = "bg-amber-500";
      const cleanName = formatClusterName(item.cluster) || "Sin nombre";
      return { region: cleanName, value: `${score.toFixed(1)}/5`, percent: `${percentValue}%`, color };
    });
  }, [rawSaludMental]);
  ...
  const metrics = useMemo(() => {
    ...
    const avgEmpleo = regionesEmpleo.length
      ? regionesEmpleo.reduce((sum, r) => sum + (r.indicadores?.[0]?.valor ? parseFloat(r.indicadores[0].valor) : 0), 0) / regionesEmpleo.length
      : 71.7;
    const tasaEmpleoVal = avgEmpleo.toFixed(1) + "%";

    const avgCongestion = regionesEmpleo.length
      ? (regionesEmpleo.reduce((sum, r) => sum + (r.congestionamento_medio ? parseFloat(r.congestionamento_medio) : 0), 0) / regionesEmpleo.length) * 100
      : 54.2;
    const congestionVal = avgCongestion.toFixed(1) + "%";

    const avgIdhmVal = regionesEducacion.length
      ? regionesEducacion.reduce((sum, r) => sum + (r.indicadores?.[0]?.valor ? parseFloat(r.indicadores[0].valor) : 0), 0) / regionesEducacion.length
      : 0.847;
    ...
    const avgSalud = regionesSalud.length
      ? regionesSalud.reduce((sum, r) => sum + (r.indicadores?.[0]?.valor ? parseFloat(r.indicadores[0].valor) : 0), 0) / regionesSalud.length
      : 75.5;
    const saludVal = avgSalud.toFixed(1) + "%";

    const totalUsuarios = regionesEmpleo.reduce((sum, r) => sum + (r.n_usuarios ? parseInt(r.n_usuarios) : 0), 0);
    const usuariosVal = totalUsuarios >= 1000 ? (totalUsuarios / 1000).toFixed(1) + "K" : totalUsuarios.toString();

    return [
      {
        title: isPortugues ? "Taxa de Emprego" : "Tasa de Empleo",
        value: tasaEmpleoVal,
        change: isPortugues ? "+2.3% vs. mês anterior" : "+2.3% vs. mes anterior",
        badge: isPortugues ? "Normal" : "Normal",
        badgeClass: "bg-green-50 text-green-700 border-green-200",
        icon: JobIcon, color: "#3b82f6",
        linePoints: "0,10 20,8 40,12 60,7 80,11 100,5",
      },
      {
        title: isPortugues ? "Congestionamento Médio de Rede" : "Congestión de Red Promedio",
        value: congestionVal,
        change: isPortugues ? "-1.2% vs. mês anterior" : "-1.2% vs. mes anterior",
      ...
      {
        title: isPortugues ? "Índice de Educação (IDHM)" : "Índice de Educación (IDHM)",
        value: educacionVal,
        change: isPortugues ? "+1.8% vs. mês anterior" : "+1.8% vs. mes anterior",
      ...
      {
        title: isPortugues ? "Cobertura Atenção Básica" : "Cobertura Atención Básica",
        value: saludVal,
        change: isPortugues ? "0.5% vs. mês anterior" : "0.5% vs. mes anterior",
      ...
      {
        title: isPortugues ? "Usuários Totais de Rede" : "Usuarios de Red Totales",
        value: usuariosVal,
        change: isPortugues
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="bg-white border border-slate-200 rounded-xl p-4 h-[135px] flex flex-col justify-between animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {metrics.map((metric, idx) => {
            ...
            return (
              <div key={idx} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col justify-between hover:shadow-sm transition-shadow animate-in fade-in duration-200">
                ...
                <div className="h-6 mt-3">
                  <svg className="w-full h-full" viewBox="0 0 100 15" preserveAspectRatio="none">
                    <polyline fill="none" stroke={metric.color} strokeWidth="1.5" points={metric.linePoints} />
                  </svg>
                </div>
              </div>
            );
      ...
      {/* 👇 grid-cols-5: mapa ocupa 3 cols, asistente 2 cols */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <BlockMap onClusterSelect={onClusterSelect} />

        {/* Asistente IA — 2 de 5 columnas */}
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between min-h-[450px] lg:col-span-2">
          <div className="flex flex-col h-full w-full justify-between flex-1">
            ...
            >
              {messages.map((msg, index) => (
                <div key={index} className={`flex gap-2.5 items-start ${msg.sender === "user" ? "flex-row-reverse" : ""}`}>
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-[10px] shrink-0 ${msg.sender === "user" ? "bg-slate-200 text-slate-700" : "bg-blue-600 text-white"}`}>
                    {msg.sender === "user" ? (isPortugues ? "Você" : "Tú") : <Bot className="w-3.5 h-3.5" />}
                  </div>
                  <div className={`rounded-xl p-3 border leading-relaxed max-w-[80%] ${msg.sender === "user" ? "bg-blue-600 text-white border-blue-600" : "bg-slate-50 text-slate-700 border-slate-150"}`}>
              ...
              {aiMutation.isPending && (
                <div className="flex gap-2.5 items-start">
                  ...
                  <div className="bg-slate-50 text-slate-500 border border-slate-100 rounded-xl p-3 flex items-center gap-1.5">
                    ...
                    <span>{isPortugues ? "IA digitando..." : "IA escribiendo..."}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {messages.length === 1 && !aiMutation.isPending && (
              <div className="space-y-2 mb-4 shrink-0">
                <button onClick={() => handleSendMessage(isPortugues ? "Quais regiões têm alto desemprego e baixa conectividade?" : "¿Qué regiones tienen alto desempleo y baja conectividad?")}
                  ...
                  {isPortugues ? "Quais regiões têm alto desemprego e baixa conectividade?" : "¿Qué regiones tienen alto desempleo y baja conectividad?"}
                </button>
                <button onClick={() => handleSendMessage(isPortugues ? "Onde faltam programas de formação?" : "¿Dónde faltan programas de formación?")}
                  className="w-full text-left text-[11px] text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 p-2 rounded-lg font-medium shadow-2xs transition-colors cursor-pointer leading-tight truncate">
                  {isPortugues ? "Onde faltam programas de formação?" : "¿Dónde faltan programas de formación?"}
                </button>
                <button onClick={() => handleSendMessage(isPortugues ? "Onde são necessários programas de saúde mental?" : "¿En donde hacen falta programas de salud mental?")}
                  className="w-full text-left text-[11px] text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 p-2 rounded-lg font-medium shadow-2xs transition-colors cursor-pointer leading-tight truncate">
                  {isPortugues ? "Onde são necessários programas de saúde mental?" : "¿En donde hacen falta programas de salud mental?"}
                </button>
                <button onClick={() => handleSendMessage(isPortugues ? "¿Os clusters com níveis de escolaridade mais baixos apresentam problemas de conectividade?" : "¿Los clusters con menos educacion presentan problemas de conectividad?")}
                  className="w-full text-left text-[11px] text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 p-2 rounded-lg font-medium shadow-2xs transition-colors cursor-pointer leading-tight truncate">
                  {isPortugues ? "¡Os clusters com níveis de escolaridade mais baixos apresentam problemas de conectividade?" : "¿Los clusters con menos educacion presentan problemas de conectividad?"}
                </button>
              </div>
            )}
            ...
            <div className="mt-auto shrink-0 pt-2 border-t border-slate-100">
              <div className="relative w-full">
                <input
                  ...
                  placeholder={isPortugues ? "Faça uma pergunta sobre os dados..." : "Haga una pregunta sobre los datos..."}
      ...
      <div className="grid grid-cols-1 md:grid-cols- gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between hover:shadow-sm transition-shadow">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-sm font-bold text-slate-800">
              {isPortugues ? "Emprego e Conectividade por Região" : "Empleo y Conectividad por Región"}
            </h3>
          ...
          <div className="flex-1 mt-4">
            <ChartContainer config={barChartConfig} className="h-[200px] w-full">
              <BarChart data={barChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="region" tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 9, fill: "#64748b" }} />
                <YAxis tickLine={false} axisLine={false} tickMargin={8} tick={{ fontSize: 9, fill: "#64748b" }} domain={[0, 100]} />
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 flex flex-col justify-between hover:shadow-sm transition-shadow">
            ...
            <div className="flex items-center gap-4 mt-4">
              <ChartContainer config={pieChartConfig} className="h-[95px] w-[95px] shrink-0">
                <PieChart>
                  ...
                  <Pie data={pieChartData} dataKey="value" nameKey="name" innerRadius={26} outerRadius={40} strokeWidth={2} stroke="#ffffff">
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-bold text-slate-700">
              <span>{isPortugues ? "Saúde Mental por Região" : "Salud Mental por Región"}</span>
              <span className="text-slate-400 font-normal">{isPortugues ? "escala 0-5" : "escala 0-5"}</span>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <button onClick={() => onTabChange?.("formaciones")} className="bg-[#eff6ff] hover:bg-[#dbeafe] border border-blue-100 rounded-xl p-4 flex items-center justify-between text-left cursor-pointer transition-all duration-200 group">
          <div>
            <h5 className="text-xs font-bold text-blue-900">{isPortugues ? "Ver Formações" : "Ver Formaciones"}</h5>
            <p className="text-[10px] text-blue-700/80 mt-1">{isPortugues ? "Programas educacionais ativos" : "Programas educativos activos"}</p>
          </div>
        ...
        <button onClick={() => onTabChange?.("empleabilidad")} className="bg-[#f0fdf4] hover:bg-[#dcfce7] border border-green-100 rounded-xl p-4 flex items-center justify-between text-left cursor-pointer transition-all duration-200 group">
          <div>
            <h5 className="text-xs font-bold text-green-900">{isPortugues ? "Análise de Emprego" : "Análisis de Empleo"}</h5>
            <p className="text-[10px] text-green-700/80 mt-1">{isPortugues ? "Oportunidades de trabalho" : "Oportunidades laborales"}</p>
          </div>
        ...
        <button onClick={() => onTabChange?.("experiencias")} className="bg-amber-50 hover:bg-amber-100 border border-amber-100 rounded-xl p-4 flex items-center justify-between text-left cursor-pointer transition-all duration-200 group">
          <div>
            <h5 className="text-xs font-bold text-amber-900">{isPortugues ? "Experiências Ativas" : "Experiencias Activas"}</h5>
            <p className="text-[10px] text-amber-800/80 mt-1">{isPortugues ? "Experiências comunitárias ativas" : "Experiencias comunitarias activas"}</p>
          </div>
        ...
        <button onClick={() => onTabChange?.("mentorias")} className="bg-[#faf5ff] hover:bg-[#f3e8ff] border border-purple-100 rounded-xl p-4 flex items-center justify-between text-left cursor-pointer transition-all duration-200 group">
          <div>
            <h5 className="text-xs font-bold text-purple-900">{isPortugues ? "Programas de Mentoria" : "Programas de Mentoría"}</h5>
            <p className="text-[10px] text-purple-700/80 mt-1">{isPortugues ? "Apoio e mentorias regionais" : "Apoyo y mentorías regionales"}</p>
          </div>
